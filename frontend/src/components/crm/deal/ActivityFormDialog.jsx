            ...
            <DialogContent className="sm:max-w-lg max-w-[95vw] p-0 gap-0 rounded-xl overflow-hidden shadow-xl border-border/50">
                ...
                <form onSubmit={handleSubmit} className="px-5 sm:px-6 py-4 sm:py-5 space-y-4 sm:space-y-5">
                    ...
                    <div className="space-y-1.5 sm:space-y-2">
                        ...
                        >
                            <SelectTrigger className="h-9 sm:h-10 rounded-lg border-border/60 focus:ring-2 focus:ring-primary/20 text-sm cursor-pointer bg-background/50">
                    ...
                    {form.type === "custom" && (
                        <div className="space-y-1.5 sm:space-y-2">
                            ...
                            <Input
                                ...
                                className="h-9 sm:h-10 rounded-lg border-border/60 focus:ring-2 focus:ring-primary/20 text-sm bg-background/50"
                            />
                        </div>
                    )}
                    ...
                    <div className="space-y-1.5 sm:space-y-2">
                        ...
                        <Textarea
                            ...
                            className="rounded-lg border-border/60 focus:ring-2 focus:ring-primary/20 text-sm resize-none bg-background/50 min-h-[80px]"
                        />
                    </div>

                    {/* Actions - More Compact */}
                    <div className="flex justify-end gap-2 sm:gap-3 pt-3 sm:pt-4 border-t border-border/30 -mx-5 sm:-mx-6 px-5 sm:px-6 py-3 sm:py-4 bg-background/20 rounded-b-xl">
                        <Button
                            type="button"
                            variant="outline"
                            onClick={() => onOpenChange(false)}
                            className="h-8 sm:h-9 px-4 sm:px-5 rounded-lg text-xs sm:text-sm font-medium border-border/60 hover:bg-muted/30"
                        >
                            Cancel
                        </Button>
                        <Button
                            type="submit"
                            disabled={isUpdating}
                            className="h-8 sm:h-9 px-4 sm:px-5 rounded-lg text-xs sm:text-sm font-medium shadow-sm hover:shadow transition-shadow"
                        >
                            {isUpdating
                                ? mode === "create"
                                    ? "Creating..."
