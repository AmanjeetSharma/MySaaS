...
function CustomerCreateDialog({ open, onOpenChange, organizationId, onCreated }) {
  ...
  const handleSubmit = async (event) => {
    ...
    onCreated(getEntityId(createdCustomer));
    onOpenChange(false);
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            <Button type="submit" disabled={isUpdating} className="w-full sm:w-auto">
              {isUpdating ? 'Saving...' : 'Create customer'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
    ...
    <Card className="overflow-hidden border-border/70 transition-all hover:-translate-y-0.5 hover:border-border hover:shadow-lg">
      <CardHeader className="space-y-3 border-b border-border/60 bg-gradient-to-br from-background to-muted/25">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <CardTitle className="truncate text-base">{customer?.name || 'Unnamed customer'}</CardTitle>
              <CardDescription className="truncate">{customer?.email || 'No email saved'}</CardDescription>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          <Badge variant="outline" className="bg-background/70">{customer?.source || 'manual'}</Badge>
          {customer?.latestInteractionAt ? (
            <Badge variant="outline" className="bg-background/70">Recent activity</Badge>
          ) : null}
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4 pt-4">
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-lg border border-border/70 bg-muted/20 p-3">
            ...
            <p className="mt-2 truncate text-sm font-medium">{customer?.email || 'Not set'}</p>
          </div>
          <div className="rounded-lg border border-border/70 bg-muted/20 p-3">
            <div className="flex items-center gap-2 text-xs uppercase tracking-wide text-muted-foreground">
            ...
            <p className="mt-2 truncate text-sm font-medium">{customer?.phone || 'Not set'}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-2 text-sm text-muted-foreground">
          ...
          <Button variant="ghost" size="sm" className="gap-1" onClick={() => navigate(`/customers/${customerId}`)}>
            Open
...
const Customers = () => {
  ...
  const { customers, getCustomers, isLoading, pagination } = useCustomerStore();
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [searchValue, setSearchValue] = useState('');

  const organizationId = getEntityId(userProfile?.activeOrganization || currentOrganization);

  useEffect(() => {
    if (!organizationId) return;
    getCustomers(organizationId).catch(() => undefined);
  }, [organizationId, getCustomers]);
      ...
      <section className="rounded-3xl border border-border/70 bg-gradient-to-br from-background via-background to-muted/35 p-5 shadow-sm sm:p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <p className="max-w-2xl text-sm text-muted-foreground sm:text-base">
                View every customer in the active workspace, open their timeline, edit their profile, or create a new deal from the same place.
              </p>
            </div>
          </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-border/70 bg-background/70 p-4">
            ...
            <p className="mt-2 text-2xl font-semibold">{pagination?.total ?? filteredCustomers.length}</p>
          </div>
          <div className="rounded-2xl border border-border/70 bg-background/70 p-4">
            <p className="text-xs uppercase tracking-wide text-muted-foreground">Visible now</p>
            <p className="mt-2 text-2xl font-semibold">{filteredCustomers.length}</p>
          </div>
          <div className="rounded-2xl border border-border/70 bg-background/70 p-4">
            <p className="text-xs uppercase tracking-wide text-muted-foreground">Active org</p>
            <p className="mt-2 truncate text-sm font-medium">
              {currentOrganization?.name || userProfile?.activeOrganization?.name || 'Organization selected'}
            </p>
          </div>
        </div>
      </section>
      ...
      {isLoading ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 6 }).map((_, index) => (
            <div key={index} className="h-56 animate-pulse rounded-3xl border border-border/70 bg-muted/30" />
          ))}
        </div>
